export default {
  "dashboard": {
    "title": "Bảng điều khiển Admin",
    "description": "Tổng quan các số liệu hệ thống và hành động nhanh.",
    "totalUsers": "Tổng số người dùng",
    "registeredUsers": "Tài khoản đã đăng ký",
    "totalTournaments": "Tổng số giải đấu",
    "createdTournaments": "Đã tạo trên nền tảng",
    "systemHealth": "Tình trạng hệ thống",
    "online": "Trực tuyến",
    "allSystemsOperational": "Tất cả hệ thống hoạt động bình thường",
    "pendingAlerts": "Cảnh báo chờ xử lý",
    "noActiveAlerts": "Không có cảnh báo nào",
    "quickActions": "Hành động nhanh",
    "manageSystemEntities": "Quản lý trực tiếp các thực thể hệ thống.",
    "manageUsers": "Quản lý Người dùng",
    "manageRoles": "Quản lý Vai trò & Phân quyền",
    "systemStatus": "Trạng thái hệ thống",
    "noSystemSnapshot": "Không có bản chụp hệ thống",
    "alertBreakdown": "{{critical}} nghiêm trọng / {{warning}} cảnh báo",
    "serviceAvailable": "Sẵn sàng",
    "serviceNeedsAttention": "Cần chú ý",
    "traffic": "Lưu lượng",
    "realtime": "Thời gian thực",
    "roomCount": "{{count}} phòng",
    "viewAllLogs": "Xem nhật ký hệ thống →"
  },
  "userManagement": {
    "title": "Quản lý Người dùng",
    "description": "Quản lý người dùng hệ thống, hồ sơ và trạng thái.",
    "addUser": "Thêm người dùng",
    "searchUsers": "Tìm kiếm người dùng...",
    "search": "Tìm kiếm",
    "user": "Người dùng",
    "contact": "Liên hệ",
    "roles": "Vai trò",
    "status": "Trạng thái",
    "actions": "Hành động",
    "loading": "Đang tải người dùng...",
    "noUsers": "Không tìm thấy người dùng nào.",
    "unknown": "Không rõ",
    "na": "N/A",
    "noPhone": "Không có SĐT",
    "roleId": "ID Vai trò: {{id}}",
    "noRoles": "Không có vai trò",
    "verified": "Đã xác minh",
    "unverified": "Chưa xác minh",
    "edit": "Chỉnh sửa",
    "delete": "Xoá",
    "rowsPerPage": "Số dòng mỗi trang:",
    "showing": "Hiển thị từ {{start}} đến {{end}} trong tổng số {{total}} người dùng",
    "previous": "Trước",
    "next": "Tiếp",
    "goTo": "Đi đến:"
  },
  "deleteUserDialog": {
    "success": "Xoá người dùng thành công",
    "error": "Không thể xoá người dùng",
    "title": "Bạn có chắc chắn không?",
    "desc1": "Hành động này không thể hoàn tác. Nó sẽ xoá vĩnh viễn người dùng",
    "desc2": "và xoá dữ liệu của họ khỏi máy chủ của chúng tôi.",
    "cancel": "Huỷ",
    "deleting": "Đang xoá...",
    "delete": "Xoá"
  },
  "userFormModal": {
    "updateSuccess": "Cập nhật người dùng thành công",
    "updateError": "Không thể cập nhật người dùng",
    "createSuccess": "Tạo người dùng thành công",
    "createError": "Không thể tạo người dùng",
    "editUser": "Chỉnh sửa Người dùng",
    "createUser": "Tạo Người dùng",
    "firstName": "Tên",
    "lastName": "Họ",
    "email": "Email",
    "password": "Mật khẩu",
    "phone": "Số điện thoại",
    "dob": "Ngày sinh",
    "gender": "Giới tính",
    "selectGender": "Chọn giới tính",
    "male": "Nam",
    "female": "Nữ",
    "other": "Khác",
    "cancel": "Huỷ",
    "saving": "Đang lưu...",
    "saveChanges": "Lưu thay đổi"
  },
  "systemLogs": {
    "title": "Nhật ký Hệ thống",
    "description": "Theo dõi tình trạng hệ thống, nhật ký API và các lệnh cron.",
    "refresh": "Làm mới",
    "auditLogs": "Nhật ký hành động",
    "cronLogs": "Nhật ký Cron",
    "auditLogsData": "Dữ liệu Nhật ký hành động",
    "auditLogsDesc": "Các yêu cầu API và dữ liệu nhật ký sẽ xuất hiện ở đây. Hiện đang chờ tích hợp nguồn dữ liệu.",
    "cronLogsEmpty": "Không có Nhật ký Cron",
    "cronLogsEmptyDesc": "Hiện tại không có nhật ký cron nào để hiển thị.",
    "systemHealthCheck": "Kiểm tra Tình trạng Hệ thống",
    "uptime": "Thời gian hoạt động",
    "alerts": "Cảnh báo",
    "cpu": "CPU",
    "resources": "Tài nguyên",
    "servicesTraffic": "Dịch vụ & Lưu lượng",
    "dbRedis": "Cơ sở dữ liệu / Redis",
    "socketCron": "Socket / Cron",
    "requests5m": "Yêu cầu (5 phút)",
    "p95Latency": "Độ trễ p95",
    "noActiveAlerts": "Không có cảnh báo nào",
    "rawJson": "JSON gốc",
    "health": "Tình trạng",
    "metrics": "Chỉ số",
    "events": "Sự kiện",
    "services": "Dịch vụ",
    "window": "Khung thời gian:",
    "1m": "1 Phút",
    "5m": "5 Phút",
    "15m": "15 Phút",
    "totalRequests": "Tổng số yêu cầu",
    "errorRate": "Tỷ lệ lỗi",
    "avgLatency": "Độ trễ trung bình",
    "slowRoutes": "Các tuyến API chậm",
    "route": "Tuyến API",
    "count": "Số lượng",
    "avgMs": "TB (ms)",
    "maxMs": "Tối đa (ms)",
    "type": "Loại:",
    "error": "Lỗi",
    "alert": "Cảnh báo",
    "cron": "Cron",
    "api": "API",
    "limit": "Giới hạn:",
    "time": "Thời gian",
    "methodPath": "Phương thức/Đường dẫn",
    "severity": "Nghiêm trọng",
    "message": "Thông báo",
    "key": "Khóa",
    "job": "Tiến trình",
    "duration": "Thời lượng",
    "noHealthData": "Không có dữ liệu tình trạng",
    "noMetricsData": "Không có dữ liệu chỉ số",
    "noEventsData": "Không có dữ liệu sự kiện",
    "noEventsFound": "Không tìm thấy sự kiện cho loại đã chọn",
    "noAuditLogs": "Không tìm thấy nhật ký hành động nào cho bộ lọc đã chọn.",
    "action": "Hành động:",
    "prev": "Trước",
    "next": "Tiếp",
    "user": "Người dùng",
    "resource": "Tài nguyên",
    "statusTime": "Trạng thái / Thời gian",
    "details": {
      "id": "ID",
      "jobName": "Tên tiến trình",
      "status": "Trạng thái",
      "level": "Cấp độ",
      "message": "Thông báo",
      "durationMs": "Thời gian chạy",
      "startedAt": "Bắt đầu lúc",
      "finishedAt": "Kết thúc lúc",
      "tournamentInfo": "Thông tin Giải đấu",
      "name": "Tên",
      "location": "Địa điểm",
      "metaData": "Dữ liệu Meta",
      "action": "Hành động",
      "resourceType": "Loại tài nguyên",
      "resourceId": "ID Tài nguyên",
      "method": "Phương thức",
      "path": "Đường dẫn",
      "route": "Tuyến API",
      "statusCode": "Mã trạng thái",
      "ip": "Địa chỉ IP",
      "userAgent": "Trình duyệt",
      "createdAt": "Tạo lúc",
      "user": "Người dùng",
      "actorUser": "Người thực hiện",
      "email": "Email",
      "requestId": "ID Yêu cầu",
      "success": "Thành công",
      "errorCode": "Mã lỗi",
      "errorMessage": "Thông báo lỗi",
      "requestMeta": "Dữ liệu Yêu cầu",
      "responseMeta": "Dữ liệu Phản hồi",
      "cronLogDetail": "Chi tiết Nhật ký Cron",
      "auditLogDetail": "Chi tiết Nhật ký hành động",
      "apiLogDetail": "Chi tiết Nhật ký API",
      "logDetail": "Chi tiết Nhật ký",
      "detailDesc": "Xem chi tiết bản ghi nhật ký hệ thống đã chọn.",
      "errorLoading": "Không thể tải chi tiết",
      "errorPrefix": "Lỗi"
    },
    "create": "Tạo mới",
    "update": "Cập nhật",
    "delete": "Xóa",
    "all": "Tất cả",
    "loadingLogs": "Đang tải nhật ký...",
    "status": "Trạng thái"
  }
};
