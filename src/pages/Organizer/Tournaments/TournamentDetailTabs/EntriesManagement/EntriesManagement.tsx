...
export default function EntriesManagement({
  ...
  const mappedEntries = (entriesData?.eligible || []).map(entry => ({
    ...
    lineupStatus: entry.isConfirmed ? "CONFIRMED" : "PENDING",
    paymentStatus: "PAID", // Placeholder since payment isn't in Entry type directly
    ...
    <div className="space-y-6 text-foreground font-sans bg-background min-h-screen p-6">
          ...
          <div className="space-y-1">
            ...
            <Select value={selectedCategoryId} onValueChange={setSelectedCategoryId}>
              ...
              <SelectContent className="bg-popover border-border text-popover-foreground">
                ...
                {categories.length === 0 && <SelectItem value="all" disabled>No categories</SelectItem>}
              </SelectContent>
            </Select>
          </div>
        </div>
      ...
      <div className="flex justify-end">
        ...
        >
          ...
          {disqualifyMutation.isPending ? "Disqualifying..." : "Mass Disqualify Ineligible"}
        </Button>
      </div>
