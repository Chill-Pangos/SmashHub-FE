...
export default function ResultsSubmissionTab() {
  ...
  const detailContent = (
    <>
      <div className="flex justify-between items-start mb-6 shrink-0">
        <div>
          <h2 className="text-2xl font-black text-foreground">
            {t("referee.resultsSubmission.finalSubmission", "Final Submission")}
          </h2>
          <p className="text-sm text-muted-foreground mt-1">
            {t("referee.resultsSubmission.reviewDesc", "Review completed match before finalizing verification.")}
          </p>
        </div>
        <div className="text-right">
          ...
          <p className="text-xs text-muted-foreground font-semibold mt-2">
            {t("referee.matchResultsReview.scheduledAt", "Scheduled:")}{" "}
        ...
        {currentMatch.subMatches && currentMatch.subMatches.length === 1 ? (
          <div className="flex flex-wrap gap-3">
            {currentMatch.subMatches[0].matchSets?.map((set: MatchSet) => (
              <div key={set.id} className="flex justify-center items-center p-3 rounded-lg border border-border bg-secondary/20 min-w-[80px]">
                <div className="flex gap-3 items-center">
                  <span className={`text-xl font-black ${set.entryAScore > set.entryBScore ? "text-primary" : "text-muted-foreground"}`}>{set.entryAScore}</span>
                  <span className="text-sm text-muted-foreground">-</span>
                  <span className={`text-xl font-black ${set.entryBScore > set.entryAScore ? "text-primary" : "text-muted-foreground"}`}>{set.entryBScore}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-3">
            {currentMatch.subMatches.map((sm: SubMatch) => (
              <div key={sm.id} className="flex flex-col gap-2 p-3 rounded-lg border border-border bg-secondary/20">
                <span className="text-sm font-semibold">{t("matchExecution.subMatchNum", { num: sm.subMatchNumber || "", defaultValue: `Sub-Match ${sm.subMatchNumber}` })}</span>
      ...
      <div className="border border-border rounded-xl p-6 flex flex-col bg-card shrink-0 mb-6">
        ...
        <div className="flex flex-col gap-5">
          <label className="flex items-start gap-3 cursor-pointer group">
            ...
            >
              {verifiedScores && <CheckCircle className="w-3 h-3 stroke-[3]" />}
            </div>
            <div>
          ...
          <label className="flex items-start gap-3 cursor-pointer group">
            ...
            >
              {signaturesLogged && <CheckCircle className="w-3 h-3 stroke-[3]" />}
            </div>
            <div>
          ...
          <label className="flex items-start gap-3 cursor-pointer group">
            ...
            >
              {conductClean && <CheckCircle className="w-3 h-3 stroke-[3]" />}
            </div>
            <div>
      ...
      <div className="flex justify-end gap-3 mt-auto pt-4 border-t border-border shrink-0">
        ...
        <Button onClick={handleApprove} disabled={approveMatchMutation.isPending || !verifiedScores || !signaturesLogged} className="shadow-[var(--auth-primary-glow)] px-8">
          <CheckCircle className="w-4 h-4 mr-2" /> {t("referee.resultsSubmission.submitOfficialCard", "Submit Official Card")}
        </Button>
      </div>
