...
export default function ResultsSubmissionTab() {
  ...
  const handleSelectMatch = (match: (typeof SUBMISSION_QUEUE)[number]) => {
    setSelectedMatch(match);
    setMobileDetailOpen(true);
  };

  // 🟢 REACT QUERY:
  // const { data: completedMatchesAwaitingSubmission } = useQuery(...) // Để render list bên trái (tương tự Chief)
  // const submitMatchMutation = useMutation({ mutationFn: submitToLedger })

  return (
    <div className="relative flex gap-6 h-[calc(100vh-140px)] overflow-hidden">
      {/* Left Sidebar: List */}
      ...
      <div className="hidden md:flex w-2/3 bg-card border border-border rounded-xl p-6 flex-col overflow-y-auto">
        ...
        <div className="bg-background border border-border rounded-xl p-8 flex justify-between items-center relative overflow-hidden mb-6">
        ...
        <div className="border border-border rounded-xl p-6 flex flex-col bg-card">
      ...
      <div
        className={`md:hidden absolute inset-0 z-20 bg-black/40 transition-opacity duration-300 ${mobileDetailOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        onClick={() => setMobileDetailOpen(false)}
      />

      <div
        className={`md:hidden absolute inset-x-0 bottom-0 z-30 max-h-[85vh] bg-card border-t border-border rounded-t-2xl p-4 overflow-y-auto transition-transform duration-300 ${mobileDetailOpen ? "translate-y-0" : "translate-y-full"}`}
      >
        ...
        <div className="bg-background border border-border rounded-xl p-4 flex justify-between items-center relative overflow-hidden mb-4">
          <div className="absolute top-2 left-1/2 -translate-x-1/2 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">
            Final
          </div>

          <div className="flex flex-col items-center gap-2 w-1/3 z-10">
            <div className="w-14 h-14 rounded-full bg-secondary border border-border"></div>
            <div className="text-center">
              <h3 className="font-bold text-sm">{selectedMatch.p1}</h3>
              <p className="text-[10px] text-muted-foreground">
                Seed #{selectedMatch.seed1}
              </p>
            </div>
            <div className="w-10 h-10 bg-card border border-border rounded-full flex items-center justify-center text-xl font-black shadow-inner">
              {selectedMatch.p1Sets}
            </div>
          </div>

          <div className="w-7 h-7 rounded-full bg-secondary flex items-center justify-center z-10 border border-border text-muted-foreground transform rotate-45">
            <span className="block transform -rotate-45 text-[10px] font-bold">
              VS
            </span>
          </div>

          <div className="flex flex-col items-center gap-2 w-1/3 z-10 opacity-70">
            <div className="w-14 h-14 rounded-full bg-secondary border border-border"></div>
            <div className="text-center">
              <h3 className="font-bold text-sm">{selectedMatch.p2}</h3>
              <p className="text-[10px] text-muted-foreground">
                Seed #{selectedMatch.seed2}
              </p>
            </div>
            <div className="w-10 h-10 bg-card border border-border rounded-full flex items-center justify-center text-xl font-black">
              {selectedMatch.p2Sets}
            </div>
          </div>
        </div>
        ...
        <div className="bg-background border border-border rounded-xl p-4 mb-4">
          ...
          <table className="w-full text-left text-sm">
            <thead className="text-[10px] text-muted-foreground font-bold border-b border-border">
              <tr>
                <th className="pb-3">Player</th>
                <th className="pb-3 text-center">S1</th>
                <th className="pb-3 text-center">S2</th>
                <th className="pb-3 text-center">S3</th>
              </tr>
            </thead>
            <tbody>
              <tr className="border-b border-border/50">
                ...
                <td className="py-3 text-center font-mono font-bold">6</td>
                <td className="py-3 text-center font-mono font-bold">3</td>
                <td className="py-3 text-center font-mono font-bold">7</td>
              </tr>
              <tr>
        ...
        <div className="border border-border rounded-xl p-4 flex flex-col bg-card">
          <h3 className="font-bold text-base mb-4">Validation Checklist</h3>
          ...
          <button
            disabled={!verifiedScores || !signaturesLogged}
            className="w-full mt-6 py-3 rounded-lg font-bold text-sm bg-primary text-primary-foreground shadow-[var(--auth-primary-glow)] disabled:opacity-50 disabled:shadow-none disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
          >
            <Check className="w-4 h-4" /> Submit Official Card
          </button>
        </div>
